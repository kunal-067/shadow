...
export const CreateCard = ({ formId, ct }) => {
...
export function JoinUsCard({ text, className }) {
    const [name, setName] = useState('');
    const [phone, setPhone] = useState('');
    const [email, setEmail] = useState('');
    const [pinCode, setPinCode] = useState('');
    ...
    return (
        <Dialog>
            <DialogTrigger className={`bg-yellow-400 py-2 text-black mt-2 px-3 rounded-md ${className}`}>
                {text ? text : 'Join Us'}
            </DialogTrigger>
            <DialogContent>
                <DialogHeader>
